...
class DecisionTree(object):
    ...
    def geerate_tree(self, X_train, y_train, max_height):
        ...
        node_queue = []  # 栈数据结构
        node_queue.append(my_tree)
        my_tree.v = my_tree.feature_name + "=?"
        print("加入:", my_tree.v)
        while node_queue:  # 栈不为空
            curnode = node_queue.pop()
            print("取出", curnode.v)
            pp = curnode.feature_name
            if curnode.high >= max_height:
                curnode.is_leaf = True
                curnode.leaf_class = curnode.train_label.value_counts().idxmax()
                continue
            unique_values = curnode.feature_value.unique()  # 最佳特征值的唯一值
            ...
            for value in unique_values:  # 对每一个特征值
                ...
                if D_X.empty:
                    ...
                    nextnode.feature_value = D_X.loc[:, best_feature_name]
