...
def getDataSet(test_size):
    ...
    diabetes = pd.read_csv('.\code\dataset\diabetes.csv', header=None).values.tolist()
    # wine=pd.read_csv('.\code\dataset\wine.data',header=None).values.tolist()
    # dataSet = pd.read_csv(r"./code/4/IRIS.csv").values.tolist()
    dataSet=diabetes
    ...
    print(test_dataset)
    # 特征值列表
    labels = ['pregnant', 'plasma_glucose', 'blood_pressure', 'triceps',
...
def createTree(dataSet, labels):
    ...
    if flagSeries:
        ...
        # 将连续值划分为不大于当前划分点和大于当前划分点两部分
        eltDataSet, gtDataSet = splitDataSetForSeries(
            dataSet, bestFeat, midSeries)
        # 得到剩下的特征标签
        subLabels = labels[:]
        # 递归处理小于划分点的子树
    ...
    # 离散值处理
        ...
        for value in uniqueVals:
            ...
            subTree = createTree(splitDataSet(
                dataSet=dataSet, axis=bestFeat, value=value), subLabels)
            # 将子树归到分叉处下
...
def classify(inputTree, featLables, testVec):
    ...
    featIndex = featLables.index(
        firstStr[:firstStr.index('=')])  # 获取决策树第一层在featLables中的位置
...
def showResult(precisionValue, recallValue, classes):
    ...
    for classItem in classes:
        print(classItem, '\t',
              precisionValue[classItem], '\t', recallValue[classItem])
...
if __name__ == '__main__':
    ...
    处理连续值时候的决策树
    """
    train_dataSet, test_dataSet, labels = getDataSet(0.2)#划分数据集
    temp_label = labels[:]  # 深拷贝
    myTree = createTree(train_dataSet, labels)
    print(myTree)
    treePlotter.createPlot(myTree)
    # matrix = calConfuMatrix(
        # myTree, temp_label, test_dataSet)  # confusion matrix
