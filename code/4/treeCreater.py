...
class DecisionTree(object):
    ...
    def fit(self, X_train, y_train, X_val=None, y_val=None):
        ...
        if self.pruning is not None and (X_val is None or y_val is None):
            raise Exception(
                'you must input X_val and y_val if you are going to pruning')
    ...
    def generate_tree(self, X, y):
        ...
        best_feature_name, best_impurity = self.choose_best_feature_to_split(
            X, y)
        ...
        if len(best_impurity) == 1:  # 离散值
            ...
            for value in unique_vals:
                my_tree.subtree[value] = self.generate_tree(
                    sub_X[feature_values == value], y[feature_values == value])
            ...
            my_tree.high = max(
                my_tree.subtree[up_part].high, my_tree.subtree[down_part].high) + 1
    ...
    def choose_best_feature_gainratio(self, X, y):
        ...
        for feature_name in features:
            is_continuous = type_of_target(X[feature_name]) == 'continuous'
            info_gain_ratio = self.info_gainRatio(
                X[feature_name], y, entD, is_continuous)
    ...
    def gini_index(self, feature, y, is_continuous=False):
        ...
        if is_continuous:
            ...
            for split_point_ in split_point_set:  # 遍历所有的分割点，寻找基尼指数最小的分割点
                ...
                gini_index = Dv1.shape[0] / m * \
                    self.gini(Dv1) + Dv2.shape[0] / m * self.gini(Dv2)
    ...
    def info_gain(self, feature, y, entD, is_continuous=False):
        ...
        if is_continuous:
            ...
            for split_point_ in split_point_set:
                ...
                Dv1 = y[feature <= split_point_]
                Dv2 = y[feature > split_point_]
                feature_ent_ = Dv1.shape[0] / m * \
                    self.entroy(Dv1) + Dv2.shape[0] / m * self.entroy(Dv2)
    ...
    def info_gainRatio(self, feature, y, entD, is_continuous=False):
        ...
        if is_continuous:
            ...
            p1 = np.sum(feature <= split_point) / \
                feature.shape[0]  # 小于或划分点的样本占比
            p2 = 1 - p1  # 大于划分点样本占比
            IV = -(p1 * np.log2(p1) + p2 * np.log2(p2))

            grain_ratio = (gain - np.log2(feature.nunique()) /
                           len(y)) / IV  # 对信息增益修正
            return [grain_ratio, split_point]
        else:
            p = pd.value_counts(feature) / feature.shape[0]  # 当前特征下 各取值样本所占比率
            IV = np.sum(-p * np.log2(p))  # 原书4.4式
            grain_ratio = self.info_gain(
                feature, y, entD, is_continuous)[0] / IV
            return [grain_ratio]
